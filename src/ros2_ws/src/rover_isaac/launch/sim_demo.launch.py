...
launch_args = [
    ...
    DeclareLaunchArgument(
        ...
        description='Set to "fastdds" or "cyclonedds" (Cyclone only supported for ROS 2 Humble and ROS 2 Jazzy) to run Isaac Sim with a specific dds type.'
    ),
    DeclareLaunchArgument(
        'gui',
        default_value='',
    ...
    DeclareLaunchArgument(
        ...
        description='Provide ROS version to use. Only Humble and Jazzy is supported.'
    ),
    DeclareLaunchArgument(
        'ros_installation_path',
        default_value='',
        description='Comma-separated list of ROS installation paths. If ROS is installed in a non-default location (as in not under /opt/ros/), provide the path to your main setup.bash file for your ROS install. (/path/to/custom/ros/install/setup.bash). Similarly add the path to your local_setup.bash file for your workspace installation. (/path/to/custom_ros_workspace/install/local_setup.bash)'
    ),
    DeclareLaunchArgument(
        'headless',
        default_value='',
        description='Set to "native" or "webrtc" to run Isaac Sim with different headless modes, if left blank, Isaac Sim will run in the regular GUI workflow. This parameter can be overridden by "standalone" parameter.'
    ),
    DeclareLaunchArgument(
        'custom_args',
        default_value='',
        description='Add any custom Isaac Sim args that you want to forward to isaac-sim.sh during run time.'
    ),
    DeclareLaunchArgument(
        'exclude_install_path',
        default_value='',
        description='Comma-separated list of installation paths to exclude from LD_LIBRARY_PATH, PYTHONPATH, and PATH environment variables.'
    ),
]
...
def launch_setup(context):
    ...
    isaacsim_node = Node(
        package='rover_isaac', executable='run_isaacsim.py',
